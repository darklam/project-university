#ifndef PAIRS_HPP
#define PAIRS_HPP
#include "CustomVector.hpp"
#include "Clique.hpp"
#include "CSV.hpp"
#include "List.hpp"
#include "HashMap.hpp"
#include "Set.hpp"
#include "string"

namespace Pairs {
    void PairsToClique(CustomVector<Pair*>* pairs, Clique* clique);
    void deleteEntries(List<Entry<Set*>*>* entries);
    List<Entry<Set*>*>* RemoveDup(List<Entry<Set*>*>* entries);
    std::string sortIds(std::string id1, std::string id2);
    HashMap<int>* createDataset(List<Entry<Set*>*>* positive, Clique *clique);
    void deleteDatasetEntries(List<Entry<int> *> * entries);
    HashMap<int>* PairsToDataset(CustomVector<Pair*>* pairs);
}


#endif // !1